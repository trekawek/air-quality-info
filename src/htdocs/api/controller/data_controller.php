...
class DataController extends \AirQualityInfo\Admin\Controller\AbstractController {
    ...
    public function kanarek() {
        $data = $this->cache->load('/api/data/kanarek.json');
        if ($data === null) {
            $data = $this->loadKanarekData();
            $this->cache->save('/api/data/kanarek.json', $data, array(
    ...
    private function loadKanarekData() {
        $data = array();
        foreach ($this->userModel->getAllUsers() as $user) {
            ...
            foreach ($deviceById as $device) {
                ...
                if (!$device['kanarek'] || $device['update_mode'] != 'push') {
                    continue;
                }
